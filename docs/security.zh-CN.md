...
Engarde 是一个多路径 TCP 中继，而不是安全隧道。它在多个边界提供准入控制，但不会为
...
active-standby 的 `resumeToken` 是每个 Flow 随机生成的 128 位恢复凭证，并且 server 还会
校验原始 peer principal 与单调 carrier generation。该 token 随 Session 帧传输，不会
提供链路加密；在不受信任路径上仍必须使用受保护传输和 `peerAuth`。
...
`redundant` 模式下，以下原有准入限制默认值为零，即不设上限：

| 配置项 | 作用域 | 效果 |
| --- | --- | --- |
| `transfer.tcp.maxStreams` | 客户端和服务器 | 限制并发逻辑 stream 数。 |
| `transfer.tcp.maxCarriersPerStream` | 服务器 | 限制 redundant stream 可附加的 carrier 数；客户端会在每个合格接口上尝试一个 carrier。 |
...
active-standby 不允许恢复资源无限增长，并为以下设置应用有限默认值：

| 配置项 | 默认值 | 效果 |
| --- | ---: | --- |
| `transfer.tcp.maxStreams` | `2048` | 两端逻辑 Flow 总上限。 |
| `transfer.tcp.maxConcurrentResumes` | `64` | Client 同时执行的恢复/迁移操作上限。 |
| `transfer.tcp.maxPendingResumes` | `128` | Client 恢复队列和 server RESUME 准入上限。 |
| `transfer.tcp.maxRecoveringStreams` | `1024` | 两端可保留 endpoint 的 recovering Flow 上限。 |
| `transfer.tcp.maxRecoveryBytes` | `536870912` | 两端恢复历史的聚合内存上限。 |

达到恢复聚合限制时，Engarde 会停止接收新 SOCKS5 Flow，并按最晚建立优先确定性终止
超额 recovering Flow。该机制限制 Engarde 自身保存的重放历史，但不能替代进程内存、
文件描述符和上游连接限制。
...
- 将 Web Manager 保留在 loopback，配置 Basic authentication，并通过 TLS/VPN/SSH
  进行远程访问。
- 在 redundant 模式为 stream、carrier 和 pending connection 设置非零限制；在
  active-standby 模式校准有限恢复默认值，并监控实际资源用量。
