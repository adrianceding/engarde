...
复用 **Session**。每条逻辑 TCP 流可按默认 `redundant` 模式在所有健康 Session 上使用
carrier，也可按 `active-standby` 模式只使用一条自适应 active carrier，并保留其他
Session 热备。
...
> [!IMPORTANT]
> Engarde 不是 MPTCP，也不会叠加多条链路的带宽。默认 `redundant` 模式在健康 carrier
> 上重复发送；`active-standby` 模式只发送一份业务数据，并在路径故障时通过热备
> Session 接管同一个 App TCP 和 target TCP。
...
- 支持默认多 carrier 冗余，以及带自适应路径评分和有界恢复的单 active carrier 模式。
- 在 carrier 切换中保留有序交付、累计 ACK 和有界重放窗口。
...
上述配置默认使用 `redundant`。要启用流量优化，必须在 client 与 server 两份配置中都
加入以下字段；其他恢复参数有有限默认值，通常不需要手工设置：
...
只有服务端已连接请求目标且至少一条 carrier 就绪后，请求才会成功。`redundant` 模式
只要仍有另一条 carrier 存活就继续传输。`active-standby` 模式会在热备 Session 上
RESUME 同一 Flow；恢复预算内不会重建 App TCP 或 target TCP，所有路径持续不可用超过
预算后才以终端错误关闭。
...
- redundant 的原有数量限制默认不限；active-standby 的 stream 和恢复资源使用有限默认值。
...
- 提供冗余或单活接管，而不是带宽聚合。
- active-standby 只能在同一 server 进程的 Session 间恢复；server 重启或所有路径超过
  恢复预算后，原有逻辑流无法继续。
