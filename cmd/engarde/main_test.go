...
func TestDefaultStartersReturnAddressErrors(t *testing.T) {
	...
	if err := defaultStartServer(context.Background(), config.Server{ListenAddr: "127.0.0.1:1", DstAddr: "bad dst"}, "", nil); err == nil {
		t.Fatal("defaultStartServer succeeded with invalid dst address")
	}
}
