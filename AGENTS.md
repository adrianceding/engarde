...
- `make test-production`：执行可重复的 PR/发布门禁，包括完整 Go 测试、`go vet` 和竞态检测。
- `make`：构建并嵌入前端，同时交叉编译 Linux、Windows 和 macOS 二进制文件，输出到 `dist/{os}/{arch}/`。
- `make web-assets-force`：强制重新生成并嵌入 Angular 资源。
- `make test-fuzz FUZZ_ITERATIONS=1000`：以固定迭代预算运行全部 Go 模糊测试目标。
- `make test-stress STRESS_RUNS=10`：显式运行随机 shuffle 压力诊断，不属于发布门禁。
- `make test-soak SOAK_DURATION=10m`：显式运行基于时间的泄漏与定时器诊断，不属于正确性门禁。
...
Go 测试文件命名为 `*_test.go`，测试函数使用 `TestXxx`、`BenchmarkXxx` 或 `FuzzXxx`；集成行为可采用 `_e2e_test.go` 等明确后缀。Angular 单元测试使用 `*.spec.ts`。修复缺陷时，应在受影响的包附近增加回归测试。核心协议行为应通过可控事件和明确状态转换进行确定性测试；时间只作为防止死锁的 watchdog，不用于决定预期结果。fuzz、随机压力和 soak 仅作补充诊断。
